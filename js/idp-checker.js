...
const idpCountries = {
  ...
  "ES": { code: "ES", name: "Spain", from_: "yes", in_: "yes_with_conditions", digital_copy: false, validity: 1, condition_using: "Printed booklet only (1-year option).", recommended_license: 40 },
  ...
  "TH": { code: "TH", name: "Thailand", from_: "yes", in_: "yes_with_conditions", digital_copy: false, validity: 1, condition_using: "Our translation is not recommended for Thailand.", recommended_license: 40 },
  ...
  "AE": { code: "AE", name: "United Arab Emirates", from_: "yes", in_: "yes_with_conditions", digital_copy: false, validity: 1, condition_using: "Printed only — digital not accepted in UAE.", recommended_license: 40 },
  ...
  "VN": { code: "VN", name: "Vietnam", from_: "yes", in_: "yes_with_conditions", digital_copy: false, validity: 1, condition_using: "Printed only — check local rules.", recommended_license: 40 },
...
const IDP_I18N = {
  en: {
    ...
    resultFor: 'Translation for {name}',
    accepted: 'Accepted',
    conditions: 'Accepted with conditions',
    notAccepted: 'Not accepted',
    printedDigital: 'Print + digital',
    printed: 'Printed booklet',
  ...
  es: {
    ...
    resultFor: 'Traducción para {name}',
    accepted: 'Aceptado',
    conditions: 'Aceptado con condiciones',
    notAccepted: 'No aceptado',
    printedDigital: 'Impreso + digital',
    printed: 'Folleto impreso',
    printedOnly: 'Solo impreso',
    years13: '1–3 años',
    year1: '1 año'
  },
  fr: {
    noCountries: 'Aucun pays trouvé',
    resultFor: 'Traduction pour {name}',
    accepted: 'Accepté',
    conditions: 'Accepté sous conditions',
    notAccepted: 'Non accepté',
    printedDigital: 'Imprimé + numérique',
    printed: 'Livret imprimé',
    printedOnly: 'Imprimé uniquement',
    years13: '1–3 ans',
    year1: '1 an'
  },
  it: {
    noCountries: 'Nessun paese trovato',
    resultFor: 'Traduzione per {name}',
    accepted: 'Accettato',
    conditions: 'Accettato con condizioni',
    notAccepted: 'Non accettato',
    printedDigital: 'Cartaceo + digitale',
    printed: 'Libretto cartaceo',
    printedOnly: 'Solo cartaceo',
    years13: '1–3 anni',
    year1: '1 anno'
  }
};
...
function idpToggle(type, event) {
  const listId = type === 'source' ? 'idpSourceList' : 'idpDestList';
  const list = document.getElementById(listId);
  // Clicking the input focuses it first (onfocus → idpOpen), then the click
  // bubbles to the trigger. Ignore that bubble so we don't immediately close.
  if (event && event.target && event.target.tagName === 'INPUT') {
    if (!list.classList.contains('open')) idpOpen(type);
    return;
  }
...
function idpUpdateResult() {
  ...
  if (!idpSelectedDest) {
    ...
    if (satText) satText.style.display = 'none';
    return;
  }
  ...
  if (ctaBtn) ctaBtn.href = IDP_CHECKOUT_URL;
}

// ===== CLOSE ON OUTSIDE CLICK =====
document.addEventListener('click', (e) => {
  const source = document.getElementById('idpSourceDropdown');
  const dest = document.getElementById('idpDestDropdown');
  if (source && !source.contains(e.target)) idpClose('source');
  if (dest && !dest.contains(e.target)) idpClose('dest');
});
